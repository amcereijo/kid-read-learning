...
const labSetupForm = document.getElementById("testLabSetup");
...
function renderLabPreview() {
  ...
  if (labState.prepared.items.length > maxPreview) {
    const li = document.createElement("li");
    li.textContent = `... y ${labState.prepared.items.length - maxPreview} mas`;
    labPreview.appendChild(li);
  }
}
...
function prepareLabData() {
  ...
  setLabStatus(
    "Datos listos",
    `${prepared.items.length} items preparados para ${labelTestType(prepared.type)} (${labelLevel(prepared.level)}).`,
    "Revisa la vista previa y pulsa Iniciar sesion cuando este todo listo."
  );
}
...
function startLabSession() {
  ...
  labProgress.textContent = `Item 1 de ${labState.active.items.length}`;
  displayCurrentLabItem();

  setLabStatus(
    "Sesion activa",
    `Prueba: ${labelTestType(labState.active.type)} (${labelLevel(labState.active.level)}).`,
    "Marca cada intento como Correcto, Error u Omitir."
  );
  ...
  if (isSpeed) {
    setLabStatus(
      ...
      "Pulsa Finalizar minuto cuando termine el tiempo o si necesitas detener antes."
    );
    beginSpeedTimer();
  }
}
...
function completeRegularFlow() {
  ...
  const advice =
    errorRate > 30
      ...
      : "Resultado dentro de margen funcional. Mantener practica regular.";
  ...
  if (labState.lastSessionSummary) {
    labState.lastSessionSummary.advice = advice;
  }
}
...
function finishSpeedSession() {
  ...
  labStimulus.textContent = "Tiempo finalizado. Introduce las palabras correctas y guarda el resultado.";
  setLabStatus(
    "Capturando resultado",
    "Escribe las palabras correctas leidas en 1 minuto.",
    "No hace falta precision perfecta; busca consistencia entre sesiones."
  );
}
...
function saveSpeedResult(event) {
  ...
  let note = "Ritmo adecuado para la edad. Mantener practica y comprension.";

  if (ppm < 50) {
    level = "Alerta de velocidad";
    note = "Por debajo de 50 ppm. Conviene evaluacion y apoyo especifico.";
  } else if (ppm < 60) {
...
<html lang="es">
  <head>
    <meta charset="UTF-8" />
    <title>Reporte de sesion - Kid Read Learning</title>
    <style>
  ...
  <body>
    <h1>Kid Read Learning</h1>
    <p>Reporte de sesion de laboratorio</p>
    ...
    <div class="note">
      <strong>Recomendacion:</strong> ${safe(summary.advice)}
    </div>
  </body>
</html>`;
...
function saveLabResultToLog() {
  ...
  saveLabToLogBtn.disabled = true;
  labAdvice.textContent = "Resultado enviado al registro. Puedes iniciar otra sesion.";
}
...
generateLabDataBtn.addEventListener("click", () => {
  prepareLabData();
});

startLabSessionBtn.addEventListener("click", () => {
  startLabSession();
});
...
toggleKidViewBtn.addEventListener("click", () => {
  ...
  toggleKidViewBtn.textContent = labState.kidFocus ? "Vista normal" : "Pantalla grande";
});
